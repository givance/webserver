...
if (!GOOGLE_CLIENT_ID || !GOOGLE_CLIENT_SECRET || !GOOGLE_REDIRECT_URI) {
  console.error(
    'Missing Google OAuth credentials in environment variables. Staff Gmail integration will not work.'
  );
}
...
export const staffGmailRouter = router({
   ...
   */
  getStaffGmailAuthUrl: protectedProcedure
   ...
   */
  disconnectStaffGmail: protectedProcedure
    .input(z.object({ staffId: z.number() }))
    .mutation(async ({ ctx, input }) => {
      // Verify staff member exists and belongs to user's organization
      const staffMember = await db.query.staff.findFirst({
