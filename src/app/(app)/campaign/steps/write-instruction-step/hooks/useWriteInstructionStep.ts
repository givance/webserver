...
export function useWriteInstructionStep(
  ...
  useEffect(() => {
    if (templatePrompt && templatePrompt.trim() && chatMessages.length === 0 && !hasAutoGeneratedFromTemplate) {
  ...
  const { remainingFromPreview, remainingFromSelected, totalRemainingDonors, canGenerateMore } = useMemo(() => {
  ...
  const saveChatHistory = useCallback(
    ...
    [sessionId, campaignName, saveDraft, previewDonorIds, selectedDonors, templateId, chatMessages]
  );
