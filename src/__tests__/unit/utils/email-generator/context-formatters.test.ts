...
describe('context-formatters', () => {
  describe('formatDonationHistoryWithIds', () => {
    ...
    it('should format donations with proper IDs', () => {
      ...
      expect(result.donationsWithIds[0].displayId).toBe('donation-1');
      expect(result.donationsWithIds[1].displayId).toBe('donation-2');
      
      // Should be sorted by date (most recent first)
      expect(result.donationsWithIds[0].date.getTime()).toBeGreaterThan(
        result.donationsWithIds[1].date.getTime()
      );
    });
    ...
    it('should format prompt string correctly', () => {
      ...
      expect(result.promptString).toContain('[donation-1]');
    ...
    it('should handle donations without projects', () => {
      const donations: DonationWithDetails[] = [
        {
          id: 1,
          amount: 15000, // $150
    ...
    it('should sort donations by date (most recent first)', () => {
      ...
      expect(result.donationsWithIds[0].id).toBe(2); // March
      expect(result.donationsWithIds[1].id).toBe(3); // February
      expect(result.donationsWithIds[2].id).toBe(1); // January
    });
  });
