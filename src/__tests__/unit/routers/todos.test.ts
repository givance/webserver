...
describe('todoRouter', () => {
  ...
  describe('create', () => {
    ...
    it('should throw UNAUTHORIZED if user is not authenticated', async () => {
      ...
      await expectTRPCError(caller.create(validInput), 'UNAUTHORIZED');
    });
    ...
    it('should validate required fields', async () => {
      ...
      // Missing title
      await expect(caller.create({ ...validInput, title: '' })).rejects.toThrow();

      // Missing type
      await expect(caller.create({ ...validInput, type: '' })).rejects.toThrow();
    });
  ...
  describe('update', () => {
    ...
    it('should throw UNAUTHORIZED if user is not authenticated', async () => {
      ...
      await expectTRPCError(caller.update({ id: 1, title: 'Updated' }), 'UNAUTHORIZED');
    });
  ...
  describe('updateMany', () => {
    ...
    it('should handle partial failures', async () => {
      ...
      // Null values are filtered out, so only 2 successes
  ...
  describe('getByOrganization', () => {
    ...
    it('should fetch todos for organization', async () => {
      ...
      expect(mockTodoService.getTodosByOrganization).toHaveBeenCalledWith('org-1', {});
    });
    ...
    it('should throw UNAUTHORIZED if user has no organization', async () => {
      ...
      await expectTRPCError(caller.getByOrganization({}), 'UNAUTHORIZED');
    });
  });
  ...
  describe('getGroupedByType', () => {
    ...
    it('should fetch todos grouped by type', async () => {
      ...
      expect(mockTodoService.getTodosGroupedByType).toHaveBeenCalledWith('org-1', undefined);
    });
    ...
    it('should exclude specified statuses', async () => {
      ...
      expect(mockTodoService.getTodosGroupedByType).toHaveBeenCalledWith('org-1', [
        'COMPLETED',
        'CANCELLED',
      ]);
    });
