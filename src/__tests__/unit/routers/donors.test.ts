...
describe("donorsRouter", () => {
  ...
  describe("getByEmail", () => {
    it("should fetch a donor by email", async () => {
      ...
      expect(mockDonorsData.getDonorByEmail).toHaveBeenCalledWith(
    ...
    it("should throw NOT_FOUND if donor doesn't exist", async () => {
      ...
      await expectTRPCError(
        caller.getByEmail({ email: "nonexistent@example.com" }),
        "NOT_FOUND"
      );
    });
    ...
    it("should validate email format", async () => {
      ...
      await expect(
        caller.getByEmail({ email: "invalid-email" })
      ).rejects.toThrow();
    });
  });
  ...
  describe("getByIds", () => {
    ...
    it("should validate maximum number of IDs", async () => {
      ...
      await expect(
        caller.getByIds({ ids: tooManyIds })
      ).rejects.toThrow();
    });
  });
  ...
  describe("create", () => {
    ...
    it("should validate required fields", async () => {
      ...
      // Missing email
      await expect(
        caller.create({ ...createInput, email: "" })
      ).rejects.toThrow();

      // Missing firstName
      await expect(
        caller.create({ ...createInput, firstName: "" })
      ).rejects.toThrow();

      // Invalid email format
      await expect(
        caller.create({ ...createInput, email: "invalid-email" })
      ).rejects.toThrow();
    });
  ...
  describe("delete", () => {
    it("should delete a donor entirely", async () => {
      ...
      await expect(
        caller.delete({ id: 1, deleteMode: "entirely" })
    ...
    it("should validate listId when deleteMode is fromList", async () => {
      ...
      await expect(
        caller.delete({ id: 1, deleteMode: "fromList" })
    ...
    it("should throw NOT_FOUND if donor doesn't exist", async () => {
      ...
      mockDonorsData.deleteDonor.mockRejectedValue(
        new Error("Donor not found")
      );
  ...
  describe("bulkDelete", () => {
    it("should delete multiple donors", async () => {
      ...
      expect(mockDonorsData.bulkDeleteDonors).toHaveBeenCalledWith(
        [1, 2, 3],
        "org-1"
      );
    });
    ...
    it("should validate maximum number of deletions", async () => {
      ...
      await expect(
        caller.bulkDelete({ ids: tooManyIds })
      ).rejects.toThrow();
    });
  });
  ...
  describe("addNote", () => {
    ...
    it("should validate note content", async () => {
      ...
      // Empty content
      await expect(
        caller.addNote({ donorId: 1, content: "" })
      ).rejects.toThrow("Note content is required");

      // Content too long
      await expect(
        caller.addNote({ donorId: 1, content: "a".repeat(5001) })
      ).rejects.toThrow();
    });
  });
