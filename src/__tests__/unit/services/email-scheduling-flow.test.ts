...
describe('Email Scheduling Flow - Pause/Resume', () => {
  ...
  describe('Complete scheduling and pause/resume flow', () => {
    it('should handle the complete flow: schedule -> pause -> resume', async () => {
      ...
      const scheduleResult = await service.scheduleEmailCampaign(
        mockSessionId,
        mockOrgId,
        mockUserId
      );
      ...
      let pauseResumeCallCount = 0;
      jest.mocked(db.select).mockImplementation(() => {
        pauseResumeCallCount++;
        if (pauseResumeCallCount === 1) {
          // First call for pause - return scheduled jobs
          return {
            from: jest.fn().mockReturnValue({
              where: jest.fn().mockResolvedValue(mockJobRecords),
            }),
          } as any;
        } else if (pauseResumeCallCount === 2) {
          // Second call for resume - return paused emails
          const pausedEmails = mockEmails.map((e) => ({ id: e.id }));
          return {
            from: jest.fn().mockReturnValue({
              where: jest.fn().mockReturnValue({
                limit: jest.fn().mockResolvedValue(pausedEmails),
              }),
            }),
          } as any;
        } else if (pauseResumeCallCount === 3) {
          // Third call for resume - return campaign schedule config
          return {
            from: jest.fn().mockReturnValue({
              where: jest.fn().mockReturnValue({
          ...
          // Fourth call and beyond - return empty for scheduleEmailCampaign
          return {
            from: jest.fn().mockReturnValue({
              where: jest.fn().mockReturnValue({
      ...
      expect(service.scheduleEmailCampaign).toHaveBeenCalledWith(
        mockSessionId,
        mockOrgId,
        mockUserId,
        null
    ...
    it('should handle partial sending before pause', async () => {
      ...
      const scheduledJobs = [{ id: 3, triggerJobId: 'trigger-3', status: 'scheduled', emailId: 3 }];
