...
test.describe('Campaign Management', () => {
  ...
  test('should navigate to campaign creation', async ({ page }) => {
    ...
    const createButton = page.locator('button:has-text("New Campaign"), button:has-text("Create Campaign"), a:has-text("Create")').first()
    ...
    if (await createButton.isVisible().catch(() => false)) {
      ...
      const currentUrl = page.url()
      expect(currentUrl).toMatch(/(campaign.*new|campaign.*create|create.*campaign)/)
    }
  })
