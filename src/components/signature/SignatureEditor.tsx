              ...
              handlePaste(view, event) {
                ...
                for (let i = 0; i < items.length; i++) {
                  ...
                  if (item.type.startsWith('image/')) {
                    ...
                    console.log('Found image item:', { type: item.type, kind: item.kind, file: file ? { name: file.name, size: file.size, type: file.type } : null });
                    ...
                    if (file && file.size > 0) {
                      ...
                      reader.onload = (e) => {
                        const result = e.target?.result as string;
                        if (result && result.length > 100) { // Basic check for valid data
                          const base64Data = result.split(',')[1];
                          const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
                          ...
                          toast.error('Image data appears to be empty. Try right-clicking the image and selecting "Copy image" instead.');
                        }
                      };
                      reader.onerror = () => {
                        toast.error('Failed to read image data. Try right-clicking the image and selecting "Copy image".');
                      };
                      reader.readAsDataURL(file);
                      return true;
                    }
                        ...
                        if (urlMatch) {
                          ...
                          .catch(error => {
                            ...
                            if (imageUrl.includes('mail.google.com')) {
                              toast.error('Gmail images cannot be copied directly. Try: 1) Right-click the image → "Copy image" → paste here, or 2) Right-click → "Save image as" → then upload the file.', {
                                duration: 8000
                              });
                            } else {
                              toast.error('Cannot fetch image from external URL. Please save the image and upload it directly.');
                            }
                          });
                        } else {
  ...
  return (
    <div className={cn("space-y-2", className)}>
      ...
      <p className="text-sm text-muted-foreground">
        Create a professional email signature with formatting, links, and images. Upload images, paste them directly (Ctrl+V), or use previously uploaded ones from the gallery.
      </p>
    </div>
  );
}